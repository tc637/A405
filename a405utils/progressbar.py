...
#https://github.com/Prooffreader/ipython_simple_progress_bar
...
class ProgressBar:
    ...
    def increment(self):
        ...
        if int(self.curr_count) > self.curr_pct:
            ...
            elif self.overflow is False:
                print(("\n* Count has gone over 100%; likely either due to:\n"
                       "  - an error in the loop_length specified when progress"
                       "_bar was instantiated\n"
                       "  - an error in the placement of the increment() function"))
                print(
                    'Elapsed time when progress bar full: {:0.1f} seconds.'.format(
